...
목적: DOWIN의 첫 유료 플랜 셀프서브 도입을 위해 Polar 기반 결제 MVP 범위, 상태 모델, 웹훅 처리, UI 노출, 검증 기준을 구현 가능한 수준으로 정리한다.
---

# Polar 결제 MVP 설계안

> 현재 상태 (2026-06-09): 이 문서는 `FREE -> STANDARD` 업그레이드형 결제 MVP를 다루는 과거 계획이다. 현재 제품 정책은 **Basic-only seat 결제**이며, 신규 구현 기준은 `docs/planning/2026.05.20-billing-entitlement-and-workspace-architecture-plan.md`를 따른다. 이 문서는 billing ledger, webhook, portal, 환불 리스크 같은 과거 설계 맥락을 확인할 때만 참고한다.

## 문서 상태

- 이 문서는 Polar billing MVP의 과거 구현 계획 문서다.
- 신규 결제 구조에서는 `FREE -> STANDARD` 업그레이드가 아니라 `workspace Basic checkout -> seat entitlement 활성화`가 기준이다.
...
- 여기서 `외부 연동 없음`은 Dowin 서버가 아직 실제 Polar API와 통신하지 않는 상태를 뜻한다.
- 즉 아직 없는 것:
...
- DOWIN는 현재 모든 워크스페이스를 `FREE`로 운영하고 있으며, `planCode` 기반 Free/Standard 경계가 일부 제품 영역에 이미 반영되어 있다.
- 현재 반영된 경계 예시:
  - `FREE` 플랜 히스토리 6개월 제한
...
- 현재 DOWIN는 복잡한 청구보다 `Free -> Team 업그레이드`, `권한 반영`, `해지/환불 시 회수`가 핵심이다.
...
- 결제 상태를 webhook 기준으로 Dowin DB에 동기화한다.
...
- `STANDARD`는 단일 Polar 상품으로 유지하되, 무제한 좌석 상품으로 운영하지 않고 Dowin 내부 멤버 상한을 둔다.
...
- Polar는 billing infrastructure와 portal을 제공하지만, 최종 entitlement 판단은 Dowin 서버가 직접 가진다.

## 5.0.1. Polar Checkout / Portal 운영 체크리스트

- 2026-04-28 기준 현재 구현은 `POST /api/billing/checkout`에서 Polar Checkout Session API로 아래 값만 전달한다.
  - `productId`
  - `external_customer_id`
  - `success_url`
  - `return_url`
  - `metadata`
- 즉, 실제 고객이 Polar 결제 화면에서 보게 되는 아래 요소는 Dowin 앱 코드가 아니라 Polar product / checkout 설정에 의해 결정된다.
  - 상품명
  - 상품 설명
  - 가격 표시
  - 정기결제 주기
  - checkout field(예: 약관/환불 조건 확인 checkbox)
  - customer portal에서 허용할 self-serve 항목

### 5.0.1.1. Polar Product 필수 설정

- Product 이름
  - `Dowin STANDARD`
- Product 설명
  - 팀 운영 확장 기능이 무엇인지 짧게 설명
  - 예:
    - `CSV export for team reporting`
    - `Longer history than FREE`
    - `Expanded team operations features`
- Pricing
  - recurring monthly
  - KRW 29,000 기준과 실제 Polar 대시보드 가격이 일치해야 한다
- Currency
  - 앱/문서에서 노출한 가격 기준과 같은 통화로 맞춘다
- Product archive 상태
  - 실제 판매할 product만 active 상태로 유지한다

### 5.0.1.2. Polar Checkout 화면 필수 점검 항목

- 결제 화면에서 사용자가 아래를 결제 전에 확인할 수 있어야 한다.
  - 무엇을 구매하는지
  - 얼마가 청구되는지
  - 월 정기결제인지
  - 자동 갱신 구조인지
- Dowin 앱 내부 고지와 Polar checkout 화면 문구가 서로 충돌하지 않아야 한다.
  - 앱: `월 29,000원`, `월 단위 정기결제`, `해지 전까지 자동 청구`
  - Polar: 동일 가격/주기/구조 표시
- Polar docs 기준 product별 checkout field를 둘 수 있고, required checkbox도 지원한다.
- DOWIN는 최소한 아래 1개는 Polar checkout에도 두는 편이 안전하다.
  - `[필수] 정기결제, 해지, 환불 조건을 확인했습니다`
- 권장 checkout field 예시
  - label:
    - `정기결제 및 환불 조건 확인`
  - helper text:
    - `월 29,000원이 자동 갱신 결제로 청구되며, 일반 해지는 다음 결제일부터 중단됩니다. 자세한 내용은 Dowin 결제 및 환불 정책을 확인해주세요.`

### 5.0.1.3. Customer Portal 필수 점검 항목

- Polar customer portal에서는 최소 아래 기능이 열려 있어야 한다.
  - 활성 구독 조회
  - 결제수단 변경
  - 구독 해지
  - 영수증 / invoice 조회
- Dowin 앱의 `결제 관리 열기`는 portal self-serve를 전제로 하므로, portal에서 해지가 막혀 있으면 문서/앱 고지와 어긋난다.
- Polar docs 기준 customer portal은 self-service cancellation과 payment method update를 제공한다.

### 5.0.1.4. 운영자 수동 점검 체크리스트

- [ ] Polar sandbox product 이름이 `Dowin STANDARD` 또는 동등하게 명확한 이름인지 확인
- [ ] Polar sandbox price가 `월 29,000원`과 일치하는지 확인
- [ ] Polar sandbox billing cycle이 monthly recurring인지 확인
- [ ] checkout 화면에서 recurring charge 문구가 명확히 보이는지 확인
- [ ] checkout field에 약관/환불 조건 확인 checkbox를 추가했는지 확인
- [ ] checkout 성공 후 Dowin `billing=success` 복귀가 정상 동작하는지 확인
- [ ] customer portal에서 결제수단 변경이 가능한지 확인
- [ ] customer portal에서 해지 예약이 가능한지 확인
- [ ] portal에서 invoice/receipt 확인이 가능한지 확인
- [ ] 앱 billing 화면의 가격/주기/해지/환불 문구와 Polar checkout/portal 표시가 일치하는지 확인

### 5.0.1.5. Dowin 코드와 Polar 설정의 책임 경계

- Dowin 코드에서 이미 고지한 것
  - billing 화면의 가격/주기/해지/환불 요약
  - 결제 및 환불 정책 페이지
  - customer portal 진입점
- Polar 대시보드에서 별도로 맞춰야 하는 것
  - product title / description
  - 실제 recurring price / currency
  - checkout field checkbox
  - portal self-serve 옵션
- 따라서 결제 릴리스 전 마지막 기준은 `코드 머지`가 아니라 `Polar sandbox 실화면 확인`이다.
...
- billing 상태는 매 요청마다 Polar API를 실시간 조회해 판단하지 않는다.
  - webhook로 append된 Dowin billing ledger를 1차 진실 원천으로 사용한다.
...
- 이유:
  - 현재 DOWIN의 플랜 게이트가 워크스페이스 단위다.
  - DOWIN의 경제적 가치도 개인 생산성보다 팀 운영/리더 개입 단위에서 더 강하게 발생한다.
...
- 예외:
  - Polar 측 운영 사유로 일시 보정이 필요하거나, DOWIN가 개별 고객 지원 차원에서 기간 유지 보상을 하기로 한 경우는 수동 처리 예외로 둔다.
...
- 기본 원칙:
  - 월 구독은 언제든 해지할 수 있다.
  - Basic seat 구독은 결제 직후 워크스페이스 대시보드, 기록 조회, 운영 기능 접근권이 즉시 열린다.
  - 따라서 이미 결제된 기간에 대해서는 원칙적으로 환불하지 않는다.
  - 일반 해지는 `현재 결제 기간 종료 후 중단`으로 처리하고, 환불과 동일시하지 않는다.
  - 고객은 Polar customer portal에서 기간 말 해지 예약을 할 수 있어야 하며, 해지 예약 후에도 이미 결제된 기간이 끝날 때까지 Basic 권한을 유지한다.
  - `no-refund` 문구를 쓰더라도 Polar가 chargeback 예방 목적으로 구매 후 60일 내 자체 판단 환불을 할 수 있다는 전제를 둔다.
- Polar 공식 문서 확인:
  - Customer Portal은 항상 제공되며 고객이 활성 구독을 직접 취소할 수 있다: https://polar.sh/docs/features/customer-portal
  - Polar subscription cancellation은 `cancel_at_period_end=true` 방식의 기간 말 해지를 지원하고, 이 경우 현재 기간 종료일까지 benefit이 유지된다: https://polar.sh/docs/guides/subscription-upgrades
  - Polar는 merchant 정책이 `no refunds`여도 chargeback 예방 목적으로 구매 후 60일 내 자체 판단 환불을 할 수 있다: https://polar.sh/docs/features/refunds
- 예외 환불 허용 조건:
  - 아래와 같은 객관적 예외 사유가 확인될 때만 예외 환불 가능
    - 관련 법령상 청약철회, 취소, 환불 권리가 인정됨
    - 중복 결제 또는 명백한 오결제
    - 결제 직후 Dowin 귀책 기술 오류로 Basic 권한이 정상 제공되지 않음
    - Polar, 결제 네트워크, chargeback 예방 처리 등 외부 결제 처리상 별도 환불이 발생함
- 예외 사유 예시:
  - 중복 결제
  - 결제 직후 기술적 오류로 Basic 권한이 정상 제공되지 않음
  - 플랜/가격 오인 등 명백한 오결제
- 환불 불가 기본값:
  - 단순 변심
  - 결제 완료 후 워크스페이스 대시보드, 기록 조회, 운영 기능 접근권이 정상 제공된 경우
  - 이미 해당 결제 기간 동안 Basic 기능을 실질적으로 사용한 경우
...
- 처리 원칙:
  ...
  - 환불 정책은 `즉시 제공되는 Basic 권한`, `이미 결제된 기간은 원칙상 환불 불가`, `일반 해지는 기간 말 종료`, `Polar의 chargeback 예방 환불 가능성`까지 한 세트로 설명해야 한다.
...
- DOWIN는 `수수료 손실`보다 `chargeback 비율 상승`을 더 큰 리스크로 본다.
...
- 이유:
  ...
  - 현재 DOWIN는 복수 플랜 청구보다 entitlement 정합성 검증이 우선이다.
...
3. 관련 webhook 수신
4. DOWIN가 `cancelAtPeriodEnd` 또는 동등 상태를 저장
5. 종료일까지 `STANDARD` 유지
...
- 다만 아래 둘은 넣을 수 있다.
  - `billing_customer_external_ref`
    ...
    - 목적:
      - Polar customer 또는 checkout metadata와 Dowin 워크스페이스를 안정적으로 매핑
  - `billing_owner_user_id`
    - nullable
...
- webhook에서 이 식별자를 사용해 Dowin 워크스페이스와 매핑한다.
...
- 다만 실시간 entitlement 판정은 webhook로 append된 ledger에서 계산된 Dowin projection을 우선 사용한다.
...
- 이유:
  - DOWIN의 권한 구조가 이미 workspace admin 중심으로 작동한다.
...
7. Polar webhook delivery와 Dowin ledger/projection 반영을 함께 확인한다
