...
목적: Dowin 저장소에 합류한 백엔드 개발자가 제품 흐름, 도메인 관계, 코드 읽기 순서, 핵심 파일 위치를 빠르게 파악할 수 있도록 돕는다.
---
...
- 현재 `pnpm tsc --noEmit`, `pnpm lint`, `pnpm test:backend`는 기본 백엔드 품질 게이트로 사용할 수 있다.
- 전체 콘솔 테스트는 `pnpm test --run`이며, Storybook browser 테스트는 별도 `pnpm test:storybook --run`으로 분리돼 있다.
...
sequenceDiagram
    ...
    participant Cookie as dowin_sid cookie
