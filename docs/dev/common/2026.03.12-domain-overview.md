---
작성일시: 2026-03-12
목적: Dowin 서비스의 전체 도메인 구조, 각 도메인의 책임 범위, 그리고 도메인 간 의존 관계를 정의한다.
---

# Dowin 전체 도메인 개요

> 현재 문서 읽기 기준:
> DOWIN는 독립적인 팀 실행 운영 제품이다.
> 이 문서에 남아 있는 `가중목`, `선행지표`, `후행지표` 같은 표현은 일부 내부 모델/히스토리 맥락을 설명하기 위한 것이며, 제품 정의의 권위는 `docs/dev/common/2026.05.09-product-positioning-and-writing-rules.md`를 따른다.
...
- **핵심 규칙**:
  ...
  - 현재 My View는 `/api/workspaces/:workspaceId/dashboard/my` 통합 조회를 우선 사용하고, 기존 점수판/로그 개별 API는 저장 후 무효화와 호환 경로로 유지한다.
...
```
1. 저녁 9시 앱 푸시 알림 수신 (Notification)
