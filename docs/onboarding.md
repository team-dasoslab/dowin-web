...
- `/contact`는 현재 `Tally` 외부 폼 동의 화면으로 동작하며, 서비스 내부 문의 저장 + Discord 운영 알림 전환은 아직 미구현이다
...
- 운영 기본기 측면에서는 외부 `Tally` 의존 문의 접수를 서비스 내부 문의 저장 + Discord 운영 알림 구조로 전환하는 작업도 백로그에 올려둔다.
