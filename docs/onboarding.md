...
### 2.3. 현재 우선순위 (2026-03-30 기준)

- 단기 우선순위는 `유료화`가 아니라 `범용 서비스화 + 무료 가치 강화`다.
- 공개 회원가입과 워크스페이스 셀프서브 진입은 현재 동작 중이며, 다음 우선순위는 핵심 여정(설정 → 기록 → 리뷰)을 제품 안에서 닫는 것이다.
- 무료 사용자 기준 핵심 여정(설정 → 기록 → 주간 리뷰)이 끊기지 않는 구조를 먼저 완성한다.
- 주간 리뷰/회고 모드, 무료 가치 측정 지표, 온보딩 카피 일관성을 다음 제품 축으로 본다.
