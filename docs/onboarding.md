...
- 점수판 공유 이미지 및 액션 아이템에 전주 대비 증감 표시 기능 추가 완료
- 제품 전반의 다국어 누락분 수정 완료
- CSV export 기능 완전 활성화 및 다운로드 화면 UI 개선 완료
...
- 그 핵심은 체크인 계열 기능이며, 기존 리더 리포트 메뉴를 재활성화하거나 확장하는 것이 아니라 새 기능으로 처음부터 기획한다. 구체적으로는 리더의 반복 직접 개입을 줄이고 `목표/성공 기준/액션 아이템 합의 -> 위험 신호 감지 -> 자동 운영 체크인 발송 -> 팀원 1탭 반응 -> 체크인 결과 보고 -> 후속 변화 확인`으로 이어지는 흐름이다.
...
- 체크인 신규 기능 기획은 `docs/planning/2026.06.16-service-led-team-checkin-plan.md`를 우선 기준으로 본다.
...
1. 온보딩 문구/플로우 정리 (CTA 용어를 `워크스페이스` 중심으로 통일)
2. 대시보드 차트/시각화 고도화
3. Push/보조 API를 OpenAPI 계약 및 공통 응답 패턴으로 정리할지 결정
4. 탈퇴/계정 삭제 정책을 문서와 구현에서 함께 확정
...
- 지금 가장 중요한 안정화 포인트는 보조 API 규약 정리와 미비한 UI 고도화 등이다
