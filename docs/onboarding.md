...
- 같은 점수판 안에서 액션 아이템만 보관/재활성화하는 사용자 노출 UX는 아직 미완료다
  - 도메인/API에는 `archive/reactivate`가 있으나, 현재 Setup 경험은 여전히 삭제 중심으로 읽히기 쉽다
  - 목표는 `기록은 남기고 현재 운영 대상에서만 제외`하는 흐름을 기본 종료 수단으로 드러내는 것이다
...
- 유료 기능 후보는 `달성률 임계치 기반 자동 리마인드`보다 서비스 주도 팀 체크인 중심으로 재정의했다. 기존 리더 리포트 메뉴는 비활성화 상태이며 신규 방향에서는 제거 대상으로 본다. 핵심은 리더가 목표/성공 기준/액션 아이템 합의와 정책 설정에 집중하고, Dowin이 `위험 신호 감지 -> 자동 운영 체크인 발송 -> 팀원 1탭 반응 -> 체크인 결과 보고 -> 후속 변화 확인`을 담당하는 것이다. 신규 기획 기준은 `docs/planning/2026.06.16-service-led-team-checkin-plan.md`를 본다
...
- 그 핵심은 서비스 주도 팀 체크인 계열 기능이며, 기존 리더 리포트 메뉴를 재활성화하거나 확장하는 것이 아니라 새 기능으로 처음부터 기획한다. 구체적으로는 리더의 반복 직접 개입을 줄이고 `목표/성공 기준/액션 아이템 합의 -> 위험 신호 감지 -> 자동 운영 체크인 발송 -> 팀원 1탭 반응 -> 체크인 결과 보고 -> 후속 변화 확인`으로 이어지는 흐름이다.
...
- 서비스 주도 팀 체크인 신규 기능 기획은 `docs/planning/2026.06.16-service-led-team-checkin-plan.md`를 우선 기준으로 본다.
