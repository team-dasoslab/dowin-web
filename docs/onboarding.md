...
- 유료 기능 후보는 `달성률 임계치 기반 자동 리마인드`보다 `리더 액션 어시스턴트` 중심으로 재정의했다. 핵심은 자동 꾸짖기가 아니라 `위험 신호 감지 -> 상황별 문장 제안 -> 리더 승인 발송 -> 팀원 1탭 반응 -> 후속 변화 확인`이며, 팀원 수용성/알림 피로 가드레일까지 포함해 `docs/planning/2026.04.14-leader-report-reminder-plan.md`를 본다
...
- 환불 악용 방지 문서 기준의 `STANDARD` usage event 저장과 반복 환불 이력의 운영 알림/수동 검토 흐름은 아직 미구현이다
