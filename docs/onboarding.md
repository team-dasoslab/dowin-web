...
- `/contact`는 현재 서비스 내부 문의 접수 폼으로 전환 중이지만, 사용자 문의 내역/결과 조회까지 포함한 최종 MVP는 아직 미완료다
...
- 운영 기본기 측면에서는 외부 `Tally` 의존 문의 접수를 서비스 내부 문의 저장 + Discord 운영 알림 + 사용자 문의 결과 조회 구조로 전환하는 작업을 백로그에 올려둔다.
