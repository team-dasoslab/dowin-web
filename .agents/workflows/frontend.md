...
### 모바일 최적화
- 모든 프론트엔드 작업은 모바일 화면을 기본 고려 범위에 포함합니다.
- 레이아웃, 터치 인터랙션, 스크롤 동선, 텍스트 가독성, 버튼/입력 영역 크기를 모바일 기준으로 점검합니다.
- 특히 대시보드, 점수판, 폼, 테이블성 UI는 작은 화면에서도 핵심 정보와 주요 액션이 우선 노출되도록 구성합니다.
- 데스크톱 전용 패턴을 그대로 축소하지 말고, 필요한 경우 모바일 전용 배치나 컴포넌트 구성을 허용합니다.
