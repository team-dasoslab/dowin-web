...
6. **API 계약 선행**: 신규 API 또는 응답/요청 구조 변경 시 `src/api-spec/openapi.yaml`을 먼저 갱신한다.
7. **테스트의 기본 축은 도메인 레이어**: 기본적으로 `validation`, `service`, `storage` 테스트에 집중하고, 라우트 테스트는 HTTP 레벨 동작이 핵심일 때만 추가한다.
...
1.  **Controller (Route Handlers)**: `src/app/api/`
    ...
    - **Cloudflare Binding**: D1 등 환경 변수 접근 시 반드시 `@opennextjs/cloudflare`의 `getCloudflareContext()`를 사용한다. (Next.js 표준 `env` 인자는 로컬 `next dev`에서 동작하지 않음)
...
5. **작업 순서**: API 구현을 시작할 때는 `domain doc 확인 -> openapi.yaml 수정 -> 테스트 작성 -> 구현` 순서를 기본으로 한다.
...
3. 신규 API나 계약 변경이 있다면 `src/api-spec/openapi.yaml`을 먼저 수정한다.

4. 테스트 파일을 구현 파일과 같은 경로에 `.test.ts` 확장자로 생성한다.
...
5. 테스트는 기본적으로 아래 순서로 작성한다.
   - `validation` 테스트: 요청 스키마와 입력 제약 검증
   - `service` 테스트: 비즈니스 규칙과 에러 케이스 검증
   - `storage` 테스트: DB 접근 계약 검증
   - `route` 테스트: 아래 경우에만 선택적으로 추가
     - 쿠키/헤더 처리
     - 파일 업로드, 스트리밍, 리다이렉트
     - 라우트 자체 분기 로직
     - 공통 유틸만으로 커버되지 않는 HTTP 동작

6. 단위 테스트와 통합 테스트(API 엔드포인트)를 분리하여 작성한다.
...
1. 구현 파일에 실제 로직을 작성한다. 이 때 다음 사항을 준수한다.
   ...
   - 라우트는 가능한 한 얇게 유지하고, 검증/세션 확인/서비스 호출/표준 응답 반환에 집중한다.
...
[ ] Phase 1: 신규/변경 API면 openapi.yaml 먼저 갱신
[ ] Phase 1: Zod 검증 스키마 정의
[ ] Phase 1: validation/service/storage 중심 테스트 작성 및 실패 케이스 확인 (Red 확인)
