...
## Phase 0: API Spec 정의 (OpenAPI)

> **API 개발의 첫 단계는 명세를 정의하는 것이다.** 이는 [Orval](https://www.orval.dev/)과 같은 도구를 통한 클라이언트 코드 자동 생성을 지원한다.

1. `docs/api/openapi.yaml` (또는 해당 도메인용 yaml) 파일을 열거나 생성한다.
2. 구현하려는 API의 Endpoints, Request/Response Schema, Error Cases를 OpenAPI 3.0 규격에 맞춰 정의한다.
3. 정의된 명세가 `docs/dev/2026.03.12-domain-*.md`의 설계와 일치하는지 확인한다.
