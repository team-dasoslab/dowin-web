---
name: wig-operations
description: Use this skill when handling WIG production operations work such as incident readiness, runbook creation, restore or rollback planning, outage response, incident communication, release smoke checks, or operational documentation for Cloudflare Workers and D1. Trigger it for requests about 운영 문서, 장애 대응, 복구 절차, 배포 롤백, DB restore, Cloudflare 장애 대응, release readiness runbooks, or 운영 체크리스트.
---
...
Use this skill for WIG operational readiness and incident-response documentation or execution guidance.
...
## WIG Operations Facts

- WIG runs on `Cloudflare Workers + D1 + OpenNext`.
...
- When the work changes WIG's operating model or entry points, update `docs/onboarding.md` and relevant indexes.
...
### 3. Ground the answer in current WIG infra
...
- Does it reflect WIG's current Cloudflare Workers + D1 setup?
...
After operations guidance is in place, move to `wig-quality-check` if you need release validation or to `wig-planning` if broader operational planning is still unresolved.
