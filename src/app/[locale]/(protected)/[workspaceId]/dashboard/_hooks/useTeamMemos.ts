...
export const useTeamMemos = ({
  ...
  const queryKey = getGetWorkspacesWorkspaceIdDashboardTeamMemosQueryKey(
    workspaceId,
    targetUserId !== null ? { targetUserId } : undefined,
  );

  const { data, error, isLoading, isFetching } = useGetWorkspacesWorkspaceIdDashboardTeamMemos(
    workspaceId,
  ...
  const createMutation = usePostWorkspacesWorkspaceIdDashboardTeamMemos();
  const resolveMutation = usePatchWorkspacesWorkspaceIdDashboardTeamMemosMemoIdResolve();
  const deleteMutation = useDeleteWorkspacesWorkspaceIdDashboardTeamMemosMemoId();
  ...
  const deleteMemo = async (memoId: number) => {
    ...
    try {
      const response = await deleteMutation.mutateAsync({ workspaceId, memoId });
