...
import { Bot, CheckCircle2, HelpCircle, X } from "lucide-react";
...
export function LeaderReport() {
  ...
  const {
    ...
    handleResolveSignal,
    submitProposal,
  } = useLeaderReportActions(workspaceId);

  const [showActiveOnly, setShowActiveOnly] = useState(true);

  const { data: reportResponse, isFetching: isPeriodLoading } =
    useGetWorkspacesWorkspaceIdTeamCheckinsReport(workspaceId, {
      weekStart: selectedWeekStart,
      activeOnly: showActiveOnly,
    });
  const { data: trendResponse, isFetching: isTrendLoading } =
    useGetWorkspacesWorkspaceIdReportsTeamTrend(workspaceId, {
      weekStart: selectedWeekStart,
      weeks: 5,
    });
  ...
  const rawTrends =
    trendResponse?.status === 200 ? trendResponse.data.trends : [];

  const weekDates = getWeekDatesFromStart(
  ...
  const [attentionFilter, setAttentionFilter] = useState<
    "ALL" | "REQUESTED" | "ADJUSTING" | "RESOLVED"
  >("ALL");
  ...
  const attentionItems = (report?.attentionItems || [])
    ...
    .filter((item) => {
      ...
      const isAdjusting =
        item.isResolved && item.resolvedProposal?.status === "PROPOSED";
      const isResolved =
        item.isResolved &&
        (item.resolvedProposal?.status === "ACCEPTED" ||
          item.resolvedProposal?.status === "DECLINED");
  ...
  const currentTrend = rawTrends?.find(
    (t) => t.weekStart === selectedWeekStart,
  );
  ...
  return (
    <div className="space-y-8 animate-dowin-in max-w-5xl mx-auto pb-20 pt-4">
      ...
      <section id="achievement" className="scroll-mt-28">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 px-2">
          <div className="bg-surface rounded-[24px] p-6 flex flex-col justify-between">
            <span className="text-[14px] text-text-secondary font-medium mb-4">
              {t("sentCount")}
            </span>
            <div>
              <div className="text-[32px] font-bold text-text-primary tracking-tight">
                {report?.summary?.sentCount || 0}
                <span className="text-[18px] text-text-muted ml-1 font-medium">
                  {t("countUnit")}
                </span>
              </div>
              <p className="text-[13px] text-text-muted mt-1">
                {t("targetCount", { n: report?.summary?.recipientCount || 0 })}
              </p>
            </div>
          </div>
          ...
          <div className="bg-surface rounded-[24px] p-6 flex flex-col justify-between">
            <span className="text-[14px] text-text-secondary font-medium mb-4">
              {t("oneTapResponseRate")}
            </span>
            <div>
              <div className="text-[32px] font-bold text-text-primary tracking-tight">
                {report?.summary?.oneTapResponseRate || 0}
                <span className="text-[18px] text-text-muted ml-1 font-medium">
                  %
                </span>
              </div>
              <p className="text-[13px] text-text-muted mt-1">
                {t("respondedCount", {
                  n: report?.summary?.respondedCount || 0,
                })}
              </p>
            </div>
          </div>
          ...
          <div className="bg-surface rounded-[24px] p-6 flex flex-col justify-between">
            ...
            <div>
              <div className="text-[32px] font-bold text-text-primary tracking-tight">
                {report?.summary?.resumedWithin24hRate || 0}
                <span className="text-[18px] text-text-muted ml-1 font-medium">
                  %
                </span>
              </div>
          ...
          <div className="bg-danger/5 rounded-[24px] p-6 flex flex-col justify-between">
            ...
            <div>
              <div className="text-[32px] font-bold text-danger tracking-tight">
                {report?.summary?.adjustmentSignalCount || 0}
                <span className="text-[18px] text-danger/60 ml-1 font-medium">
                  {t("countUnit")}
                </span>
              </div>
              <p className="text-[13px] text-danger/70 mt-1">
                {t("needsHelp")}
              </p>
            </div>
          </div>
      ...
      <div className="flex flex-col h-full px-2">
        <h3 className="text-[20px] font-bold text-text-primary mb-5 px-2">
          최근 {trendPoints.length || 5}주 달성률
        </h3>
          ...
          <div className="flex items-center gap-8 mb-6 px-2">
            <div>
              ...
              <div className="text-[28px] font-black tracking-tight text-text-primary">
                ...
                <span className="text-[16px] text-text-muted ml-0.5">%</span>
              </div>
            </div>
                  ...
                  <div className="flex gap-4 items-center">
                    ...
                    <div className="flex-1 min-w-0 py-0.5">
                      <div className="flex justify-between items-start w-full mb-0.5">
                        <span
                          className={`text-[14px] font-medium truncate pr-2 ${!signal.isResolved ? "text-primary" : "text-[#8B95A1]"}`}
                        >
                          {signal.leadMeasureName}
                        </span>
                        <span className="text-[12px] text-[#8B95A1] shrink-0 mt-0.5">
                          {formatDateRelative(signal.createdAt)}
                        </span>
                      </div>

                      <div
                        className={`flex items-center gap-1.5 text-[15px] leading-snug tracking-tight truncate ${!signal.isResolved ? "font-bold text-[#191F28]" : "font-medium text-[#4E5968]"}`}
                      >
                        <span className="truncate">
                          {signal.memberNickname}
                        </span>
                        <span className="text-[#B0B8C1] text-[12px] font-bold">
                          •
                        </span>
                        <span
                          className={`text-[14px] font-bold ${signal.isResolved ? "text-[#8B95A1]" : signal.signalType === "BLOCKED" ? "text-red-500" : "text-primary"}`}
                        >
                          {signal.isResolved
                            ? signal.resolvedProposal?.status === "ACCEPTED"
                              ? t("statusAdjustDone")
                              : signal.resolvedProposal?.status === "DECLINED"
                                ? t("statusDeclined")
                                : t("statusAdjusting")
          ...
          {activityLog.length === 0 ? (
            ...
            <div className="relative pl-6 space-y-8">
              {activityLog.map((activity, i) => {
                ...
                return (
                  <div
                    key={activity.checkinId! + i}
                    className="relative flex items-start"
                  >
                    <div
                      className={`absolute -left-6 flex items-center justify-center w-10 h-10 rounded-full z-10 transition-transform ${activity.type === "CHECKIN_SENT" ? "bg-primary" : "bg-success"}`}
                    >
                      {activity.type === "CHECKIN_SENT" ? (
                      ...
                      <p className="text-[15px] text-text-primary font-bold leading-snug tracking-tight mb-1">
                        {activity.type === "CHECKIN_SENT"
                          ? t("historySent", {
                              ...
                              measureName: activity.leadMeasureName || "",
                            })
                          : t("historyResponded", {
                              nickname: activity.memberNickname || "",
                              measureName: activity.leadMeasureName || "",
      ...
      {activeSignalModal &&
        (() => {
          const signal = attentionItems.find(
            (s) => s.responseId === activeSignalModal,
          );
            ...
            >
              ...
              >
                <div className="px-6 py-5 flex items-start shrink-0 bg-white z-10 rounded-t-[32px] border-b border-[#F2F4F6] relative">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-0.5">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span
                            className={`text-[15px] font-bold ${signal.isResolved ? "text-[#8B95A1]" : signal.signalType === "BLOCKED" ? "text-red-500" : "text-primary"}`}
                          >
                            {signal.isResolved
                              ? signal.resolvedProposal?.status === "ACCEPTED"
                                ? t("statusAdjustDone")
                  ...
                  <button
                    onClick={closeModal}
                    className="absolute top-5 right-5 p-2 hover:bg-[#F2F4F6] rounded-full transition-colors text-[#8B95A1] shrink-0"
                  >
                    <X className="w-6 h-6" />
                  </button>
                </div>
                      ...
                      <div className="flex flex-col gap-2 mt-4">
                        <div className="flex items-center justify-between px-1">
                          ...
                          <span className="text-[13px] font-medium text-primary">
                            {formatDateRelative(
                              signal.resolvedProposal.createdAt,
                            )}
                          </span>
                        </div>
                        <div className="bg-primary/10 rounded-[16px] p-5">
                          {signal.resolvedProposal.leaderNote && (
                            <p
                              className={`text-[15px] text-[#191F28] leading-relaxed font-medium mb-4 whitespace-pre-wrap`}
                            >
                              {signal.resolvedProposal.leaderNote}
                            </p>
                          )}
                          <div
                            className={
                              signal.resolvedProposal.leaderNote
                                ? "pt-4 border-t border-primary/15"
                                : ""
                            }
                          >
                            <p className="text-[16px] text-[#191F28] leading-relaxed font-bold">
                              {signal.resolvedProposal.actionType ===
                              "CHANGE_TARGET_COUNT"
                                ? "실행 횟수 조정 제안"
                                : signal.resolvedProposal.actionType ===
                                    "ARCHIVE_ACTION_ITEM"
                        ...
                        <div className="flex flex-col gap-2 mt-4">
                          ...
                          <div className="bg-[#F2F4F6] rounded-[16px] p-5">
                            <p className="text-[15px] text-[#333D4B] leading-relaxed font-medium">
                              {signal.resolvedProposal.status === "ACCEPTED"
                                ? t("proposalAcceptedMsg")
                                : t("proposalDeclinedMsg")}
                            </p>
                          </div>
                        </div>
                      )}
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <div className="bg-[#F2F4F6]/50 rounded-[16px] p-3.5 border border-border/30">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                          <span className="text-[13px] font-bold text-[#8B95A1] shrink-0 px-1">
                            액션 제안 (필수)
                          </span>
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            <button
                              onClick={() => {
                                setProposalType(
                                  proposalType === "archive" ? null : "archive",
                                );
                              }}
                        ...
                        {proposalType === "update_metric" && (
                          <div className="mt-3.5 pt-3.5 border-t border-border/40 flex items-center gap-2 animate-in fade-in slide-in-from-top-1 px-1">
                            <span className="text-[14px] text-[#333D4B] font-medium">
                              실행 목표를
                            </span>
                            <Input
                              type="number"
                        ...
                        {proposalType === "replace_action_item" && (
                          <div className="mt-3.5 pt-3.5 border-t border-border/40 flex items-center gap-2 animate-in fade-in slide-in-from-top-1 px-1">
                            <Input
                              ...
                              onChange={(e) =>
                                setProposalReplacementText(e.target.value)
                              }
...
function WeeklyRateTrendChart({
  points,
}: {
  points: { label: string; rate: number; executionRate?: number }[];
}) {
      ...
      >
        ...
        <Tooltip
          ...
          content={({ active, payload }) => {
            ...
            const executionRatePayload = payload.find(
              (p) => p.dataKey === "executionRate",
            );
            return (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <p className="font-mono text-[12px] font-bold text-white/70">
                      ...
                      <span className="text-white ml-1">
                        {executionRatePayload.value}%
                      </span>
                    </p>
                  </div>
                )}
                <div
                  className={`flex items-center gap-2 ${executionRatePayload ? "mt-1" : ""}`}
                >
                  <div
                    className="w-2 h-2 rounded-full"
                    style={{ backgroundColor: "#3a64c7" }}
                  />
                  <p className="font-mono text-[12px] font-bold text-white/70">
                    달성률{" "}
                    <span className="text-white ml-1">
