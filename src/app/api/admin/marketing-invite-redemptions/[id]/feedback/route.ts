...
export const PATCH = withErrorHandler(async (request: Request, ctx) => {
  ...
  const parsedParams = marketingInviteRedemptionParamsSchema.safeParse(await ctx.params);
  if (!parsedParams.success) {
    return await apiError("VALIDATION_ERROR", parsedParams.error.flatten().fieldErrors);
  }

  const parsedBody = marketingInviteRedemptionFeedbackUpdateSchema.safeParse(await request.json());
  ...
  ).updateRedemptionFeedback(session.adminUserId, parsedParams.data.id, parsedBody.data);
