...
export const GET = withErrorHandler(async (_request: Request, ctx) => {
  ...
  const parsedParams = contactInquiryIdParamsSchema.safeParse(await ctx.params);
...
export const PATCH = withErrorHandler(async (request: Request, ctx) => {
  ...
  const parsedBody = adminContactInquiryUpdateSchema.safeParse(await request.json());

  if (!parsedBody.success) {
    return await apiError("VALIDATION_ERROR", parsedBody.error.flatten().fieldErrors);
  }

  const result = await createService(db).updateAdminInquiry(parsedParams.data.id, {
