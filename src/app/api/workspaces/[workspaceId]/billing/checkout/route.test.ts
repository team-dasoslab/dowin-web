...
describe("POST /api/billing/checkout", () => {
  ...
  it("세션이 없으면 401을 반환한다", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/billing/checkout", { method: "POST" }), { params: Promise.resolve({ workspaceId: "ws_uid", id: "1" }) } as unknown as { params: Promise<Record<string, string>> }
    );
  ...
  it("Idempotency-Key가 없으면 422를 반환한다", async () => {
    ...
    const response = await POST(
      ...
      }), { params: Promise.resolve({ workspaceId: "ws_uid", id: "1" }) } as unknown as { params: Promise<Record<string, string>> }
    );
