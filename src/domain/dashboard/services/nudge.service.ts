...
export class NudgeService {
  ...
  async sendNudge(
    ...
    const senderProfile = await this.profileStorage.findProfileByUserId(context.userId);
    const senderNickname = senderProfile?.nickname ?? "팀원";

    const targetProfile = await this.profileStorage.findProfileByUserId(input.toUserId);
    const targetLocale = (targetProfile?.locale as Locale) ?? "ko";
    const t = messages[targetLocale] ?? messages.ko;

    const tokens = await this.notificationStorage.findActiveTokensByUserId(input.toUserId);
    ...
    if (delivery.disabledTokens.length > 0) {
      await this.notificationStorage.disableDevicePushTokens(delivery.disabledTokens);
    }
  }
}
