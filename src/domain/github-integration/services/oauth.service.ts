  ...
  return {
    ...
    async handleOAuthCallback(code: string, state: string) {
      ...
      const installation = installationsData.installations.find(
        (inst) => inst.id.toString() === targetInstallationId,
      );
      ...
      // 4. Use App JWT + installation access token to fetch the authoritative repo list.
      //    This follows the plan (step 10): user token verifies access, App JWT fetches the list.
      //    normalizePem handles literal \n sequences that Cloudflare env vars may contain.
