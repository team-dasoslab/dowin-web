  ...
  render: () => (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
      {(Object.keys(ICON_MAP) as IconName[]).sort().map((name) => (
        <div
          key={name}
          className="flex flex-col items-center justify-center p-4 border border-border rounded-content bg-surface transition-all group"
        >
          <DowinIcon
            name={name}
            size="24px"
            className="mb-2 text-text-muted transition-colors"
          />
          <span className="text-[10px] font-mono text-text-muted transition-colors truncate w-full text-center">
            {name}
          </span>
        </div>
      ))}
    ...
    <div className="flex items-end gap-6">
      <div className="flex flex-col items-center gap-2">
        <DowinIcon
          ...
          className="text-primary"
        />
        <span className="text-[10px] text-text-muted">16px</span>
      </div>
      <div className="flex flex-col items-center gap-2">
        <DowinIcon
          name="domain-flash-active"
